...
export function initExamplesAndUseCases(): void {
  ...
  if (basicSlot) {
    createEmoji('mochi', { target: basicSlot, size: 100, ambientParticles: false });
  }

  const progSlot = document.getElementById('example-prog-slot');
  if (progSlot) {
    const progEmoji = createEmoji('mochi', { target: progSlot, size: 100, ambientParticles: false });
  ...
  if (decorSlot) {
    createEmoji('nima', { target: decorSlot, size: 100, interactive: false, ambientParticles: false });
  }

  // Casos de uso (9 mockups)
  const slotBtn = document.getElementById('uc-slot-btn');
  if (slotBtn) createEmoji('mochi', { target: slotBtn, size: 26, draggable: false, ambientParticles: false });

  const slotLogo = document.getElementById('uc-slot-logo');
  if (slotLogo) createEmoji('zumi', { target: slotLogo, size: 30, draggable: false, ambientParticles: false });
  ...
  if (slotToast) createEmoji('suri', { target: slotToast, size: 30, draggable: false, interactive: false, ambientParticles: false });

  const slotChat = document.getElementById('uc-slot-chat');
  if (slotChat) createEmoji('cota', { target: slotChat, size: 44, draggable: false, ambientParticles: false });

  const slotLoading = document.getElementById('uc-slot-loading');
  if (slotLoading) createEmoji('dozy', { target: slotLoading, size: 40, draggable: false, interactive: false, ambientParticles: false });

  const slotForm = document.getElementById('uc-slot-form');
  if (slotForm) createEmoji('nima', { target: slotForm, size: 26, draggable: false, interactive: false, ambientParticles: false });

  const slotFloat = document.getElementById('uc-slot-float');
  if (slotFloat) createEmoji('pip', { target: slotFloat, size: 36, draggable: false, ambientParticles: false });

  const slotAvatar = document.getElementById('uc-slot-avatar');
  if (slotAvatar) createEmoji('snug', { target: slotAvatar, size: 40, draggable: false, ambientParticles: false });
}
